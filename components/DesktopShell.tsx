// frontend/components/DesktopShell.tsx — the desktop sidebar + top bar.
// Extracted from PageShell.tsx (the desktop portion of the original
// component). The sidebar (220px open / 60px collapsed) and the top
// bar (with breadcrumb or headerTitle) are in one tree here; this tree
// is only rendered on viewports >= 768px (gated by the PageShell
...
export function DesktopShell({
  ...
  // matching nav item's label, else the group default passed via `headerTitle`.
  // (The mobile shell's `activeTab`/`extraTitle` resolution isn't needed here:
  // the desktop top bar shows `headerTitle` for top-level routes, with the
  // breadcrumb taking over on detail routes.)
  const pageTitle =
