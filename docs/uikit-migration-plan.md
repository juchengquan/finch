...
> | **4 — opportunistic** | **Settings done.** Its root was the last hosted screen that was pure wiring — ten rows whose destinations all went native in Phase 2 — so one `SettingsListVC` now serves the compact tab root and the iPad two-column secondary, and the tab hosts no SwiftUI at all. `SettingsTab.swift` stays: macOS renders it, and `-uikitActivity NO` needs it as the control half of the dual `NavigationUITests` run. **Insights is what remains** — ~734 lines of cards, charts and a customize sheet, deliberately not bundled with Settings so a toolbar regression could not hide in the diff. Still optional by design. |
>
