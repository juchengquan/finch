...
final class CounterpartyAdminTests: XCTestCase {
    func test_crud_verify_and_projection() throws {
        ...
        var cp = try Projection.counterparties(dbQueue: q).first { $0.id == "cp1" }
        ...
        cp = try Projection.counterparties(dbQueue: q).first { $0.id == "cp1" }
        ...
        XCTAssertTrue(try Projection.counterparties(dbQueue: q).isEmpty)
    }
}
