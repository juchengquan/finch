...
final class BudgetsListVC: UIViewController {
    ...
    /// The one apply on this screen that animates. `BudgetsTab` — the SwiftUI screen
    /// this was converted from — wraps the same toggle in `withAnimation`; the
    /// conversion dropped it. Mirrors `AccountsListVC.toggleGroup` exactly, including
    /// reconfiguring only this group's row so the animated apply has nothing else to
    /// crossfade.
