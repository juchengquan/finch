...
// "block". (Verified on the simulator with 30fps captures; it happens on every
// tab pair, worst when the source is scrolled.) The dissolve lives in the backing
// `UITabBarController` — SwiftUI's own `.transaction`/animation controls don't
// reach it — so we suppress it through the tab controller's PUBLIC delegate hook:
