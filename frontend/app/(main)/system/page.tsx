...
export default function SystemPage() {
  ...
  const openAdd = () => { setDraft({ ...EMPTY, date: today }); setOpen(true); };
  const submit = () => {
  ...
  return (
    <MobilePage
      header={<ScreenHeader title="System" trailing={<IconButton icon="plus" aria-label="Add rate" onClick={openAdd} />} />}
    >
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1.5">
                ...
                <Select value={draft.currency} onValueChange={(v) => setDraft({ ...draft, currency: v })}>
                  ...
                  <SelectContent>
                    {Object.keys(CURRENCIES).filter((c) => c !== 'SGD').map((c) => (
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <Input id="rate-date" type="date" value={draft.date} onChange={(e) => setDraft({ ...draft, date: e.target.value })} />
              </div>
            </div>
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="rate-value">Rate (1 {draft.currency} = ? SGD)</Label>
              <Input id="rate-value" type="number" inputMode="decimal" step="0.000001" value={draft.rate} onChange={(e) => setDraft({ ...draft, rate: e.target.value })} placeholder="0.00872" autoFocus />
            </div>
            <div className="flex flex-col gap-1.5">
              <Label>Source</Label>
