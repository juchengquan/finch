...
/**
 * Exchange-rate management: a collapsed summary line that expands into the
 * per-currency rate list plus an add/delete dialog. Rates are USD-pivoted
...
export function ExchangeRates() {
  ...
  const byCurrency = currencies.map((cur) => {
    ...
    return { cur, latest: series[series.length - 1] };
  });
  const lastUpdated = byCurrency.reduce<string | null>(
    (max, r) => (max && max > r.latest.date ? max : r.latest.date),
    null,
  );
  ...
  return (
    <div>
      {/* Collapsed summary — the rate list is reference data most sessions
          never touch, so it stays one line until expanded. */}
      <button
        type="button"
      ...
      {expanded && (
        <>
          <div className="bg-card border-border mt-3 overflow-hidden rounded-xl border">
            {byCurrency.map((row, i) => (
              <div key={row.cur} className={cn('flex items-center gap-3 px-3.5 py-2.5', i && 'border-border border-t')}>
                <div className="w-10 font-mono text-sm font-semibold">{row.cur}</div>
                <div className="text-muted-foreground flex-1 font-mono text-[11px]">{row.latest.date}</div>
                <span
                  className={cn(
