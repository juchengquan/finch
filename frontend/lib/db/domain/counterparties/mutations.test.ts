...
test('counterparties: list, search, verify, alias', async () => {
  ...
  expect((await listCounterparties(exec)).length).toBeGreaterThan(0);

  const grab = await searchCounterparties(exec, 'GRAB');
  expect(grab.some((c) => c.name === 'Grab')).toBe(true);

  const donki = (await listCounterparties(exec)).find((c) => c.id === 'cp-04')!;
  expect(donki.verified).toBe(false);
  await verifyCounterparty(exec, 'cp-04');
  const after = (await listCounterparties(exec)).find((c) => c.id === 'cp-04')!;
...
test('createCounterparty inserts an unverified merchant', async () => {
  const exec = await seededAndAudited();
  await applyMutation(exec, 'createCounterparty', { id: 'cp-new', name: 'Starbucks' });
  const cp = (await listCounterparties(exec)).find((c) => c.id === 'cp-new')!;
