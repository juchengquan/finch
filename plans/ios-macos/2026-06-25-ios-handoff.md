...
**Branch context:** all work targets `feat/frontend`. Snapshot taken @ `c4cca05` (#435 merged). **Web→iOS parity is complete** (original inventory closed at #355/#358; the two later web-side gaps closed at #427/#431) — and iOS is now **ahead of the web for the first time**: the budget cycle-history chart (#435) has no web counterpart. The "deepen widgets" sub-project, macOS-parity Phases 1–4, and the full Watch sub-project are also complete.
...
- 0 open PRs at this snapshot (@`c4cca05`). Both sessions push frequently — **`git fetch` + re-read the inventory/this doc before picking**, and check `gh pr list --base feat/frontend --state open` (inspect the PR's *files*, not just the title).
