...
**A leg carries no currency at all — it inherits its account's.** An earlier draft of this section claimed two conflicting defaults at `Transactions.swift:295` and `:326` and concluded the payload must require an explicit per-leg currency. **That was a misreading and is withdrawn.** There is no per-leg currency to default: `AccountShare` is `{accountId, amount}` (`:253-256`), and each share converts from **its own account's** currency, looked up per share (`:287-290`). The two lines cited are about different things — `:295` is the *stated total's* currency, used only for the shares-vs-total cross-check (and the comment at `:292-294` says exactly why an omitted one means ledger base there), while `:326` is the single-account input currency.

**So `saveTransaction`'s legs stay `{accountId, amount}`,** and the design gets simpler rather than stricter. The top-level `currency` keeps its existing meaning: the currency of the stated total, for the cross-check only. **Mixed-currency split tender already works this way and is tested** (`MultiAccountAddTests.test_addTransaction_mixedEURandUSDAccounts_validSplit_isAccepted`).
...
- **Currency required per leg**, no default — the two existing defaults disagree (`:295` base vs `:326` account)
