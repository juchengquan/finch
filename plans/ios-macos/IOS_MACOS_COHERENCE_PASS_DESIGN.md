...
**Goal** — Improve the **navigability and consistency** of the 16 iOS/macOS design specs (`plans/ios-macos/IOS_MACOS_*.md`, ~13,000 lines after 3 grill passes + the wire-format annex). The 16 specs cover Phases 1.0-8 + Roadmap + Plan + Wire-format annex, with cross-references throughout. After this pass:
...
### §2.1 — `plans/ios-macos/IOS_MACOS_INDEX.md` (new file, ~400 lines)
...
1. **Phase A**: Add `plans/ios-macos/IOS_MACOS_INDEX.md` (the new file, ~400 lines)
...
Adds:
- plans/ios-macos/IOS_MACOS_INDEX.md (~400 lines) — new glossary + location
...
- `rg "see §" plans/ios-macos/IOS_MACOS_*.md` — should return only lines that include a file name (e.g., "see `IOS_MACOS_<FILE>.md` §X")
- `rg "^## §" plans/ios-macos/IOS_MACOS_PHASE_*_DESIGN.md | wc -l` — should be 15 × 8 = 120 (or close, if a section is marked "Not applicable")
- `ls plans/ios-macos/IOS_MACOS_INDEX.md` — should exist
...
3. **8-section template test**: every phase spec has 8 sections. Run `rg "^## §" plans/ios-macos/IOS_MACOS_PHASE_*_DESIGN.md | awk -F: '{print $1}' | sort | uniq -c` — should return 15 lines, each with count=8.
4. **Cross-ref test**: `rg "see §" plans/ios-macos/IOS_MACOS_*.md` — should return only lines that include a file name. A "bare" `see §X` (no file name) is a missed backfill.
