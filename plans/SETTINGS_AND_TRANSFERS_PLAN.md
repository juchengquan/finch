...
## 4. In-flight follow-on — transfer time

`lib/db/queries/transfers.ts` (uncommitted) adds `time` to the `Transfer` read
shape (`MAX(t.time)`) and to `TransferPatch` (`UPDATE … SET time`), so the
edit-transfer flow can round-trip the time-of-day that `29e286a` started
writing. **Land this before the `/transfers` detail relies on transfer time.**
...
- [ ] Transfer `time` round-trips through `/transfers` edit (follow-on §4).
