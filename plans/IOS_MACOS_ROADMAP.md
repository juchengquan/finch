...
- **App Intents / Siri**: "Add a $6 coffee to Personal" → an
  `AddTransaction` intent over `FinchCore` (dispatches through
  the Phase 2 chokepoint — there is no other write path)
- **Share Extension receipts**: the long-deferred receipt-photo
  feature lands here. Share a photo/PDF into finch from Photos,
  Safari, Mail; create / attach to an entry. Writes to
  `attachments/<entry_id>/...` (the path structure is stable
  across the DE cutover per the plan's §2.5.1)
- **Spotlight indexing**: index transactions / merchants /
  accounts via `CoreSpotlight` so system search jumps into finch
  (the iOS analogue of the web's ⌘K)
- **Notifications**: local notifications for scheduled items due,
  budget warning thresholds (`warning_pct`), anomaly flags, the
  Sunday weekly digest
- **Biometric lock**: optional Face ID / Touch ID / Optic ID gate
  on launch and on sensitive actions (export, delete-all,
...
- App launch on a fresh device requires Face ID; export still
  works after Face ID auth; the data is `completeUnlessOpen` at
  rest (per the plan's §10)
...
- The Share Extension runs in a separate process; it needs to
  ...
  container for the Share Extension's writes?
- Biometric lock on launch vs. on sensitive actions only: the
  ...
  "lock after 5 minutes idle" vs. "lock on background".)
