...
// Each account expands to reveal recent activity. By default the first
// account is expanded so the merged Accounts + Activity story is visible.
// ─────────────────────────────────────────────────────────────
function ScreenAccounts({ th }) {
  const total = MOCK.accounts.reduce((s, a) => s + a.balance, 0);
  const [open, setOpen] = React.useState({ [MOCK.accounts[0].id]: true });
  const toggle = (id) => setOpen((o) => ({ ...o, [id]: !o[id] }));

  // Group transactions by account
  const txByAcct = {};
  MOCK.transactions.forEach((tx) => {
    (txByAcct[tx.account] = txByAcct[tx.account] || []).push(tx);
  });
      ...
      <div style={{ height: '100%', overflowY: 'auto', paddingBottom: 110 }}>
        ...
        <div style={{ padding: '0 20px', display: 'flex', flexDirection: 'column', gap: 10 }}>
          {MOCK.accounts.map((a) => {
            ...
            const txs = (txByAcct[a.id] || []).slice(0, 4);
            return (
              <div key={a.id} style={{ background: th.card, border: `1px solid ${th.line}`, borderRadius: 14, overflow: 'hidden' }}>
                {/* Account header row */}
                <div onClick={() => toggle(a.id)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '14px 16px', cursor: 'pointer' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontFamily: th.mono, fontSize: 10, color: th.muted, letterSpacing: 0.5, marginTop: 2 }}>•••• {a.last4} · {a.type.toUpperCase()}</div>
                  </div>
                  <div style={{ textAlign: 'right', marginRight: 8 }}>
                    <div style={{ fontFamily: th.display, fontSize: 20, letterSpacing: -0.4, lineHeight: 1, color: a.balance < 0 ? th.neg : th.ink }}>
                      {a.balance < 0 ? '−' : ''}<Money value={Math.abs(a.balance)} currency={th.currency} mono={false} style={{ fontFamily: th.display }}/>
                    </div>
                    <div style={{ fontFamily: th.mono, fontSize: 9, color: th.muted, letterSpacing: 0.5, marginTop: 4 }}>
                      {txs.length} TX · 30D
                    </div>
                  </div>
                  <Icon name={isOpen ? 'chev-u' : 'chev-d'} size={14} style={{ color: th.muted }}/>
                </div>

                {/* Expanded — recent activity */}
                {isOpen && (
                  <div style={{ borderTop: `0.5px solid ${th.line}`, background: th.paper }}>
                    {/* Sparkline strip */}
                    <div style={{ padding: '14px 16px 6px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
                      <div style={{ fontFamily: th.mono, fontSize: 10, letterSpacing: 1.2, color: th.muted }}>30D MOVEMENT</div>
                      <Sparkline values={[20,30,18,42,52,38,45,60,48,55,72,68,60,75]} width={140} height={28} color={a.balance < 0 ? th.neg : th.accent} stroke={1.4}/>
                    </div>
                    {/* Recent transactions */}
                    <div style={{ padding: '0 16px' }}>
                      {txs.map((tx, i) => {
                        ...
                        const inc = tx.amount > 0;
                        return (
                          <div key={tx.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 0', borderTop: i ? `0.5px solid ${th.line}` : `0.5px solid ${th.line}` }}>
                            <MerchantGlyph name={tx.merchant} size={28} hue={cat.hue}/>
                            <div style={{ flex: 1, minWidth: 0 }}>
                              ...
                              <div style={{ fontSize: 10, color: th.muted, marginTop: 1 }}>
                                {tx.date.slice(5).replace('-','/')} · {cat.name || 'Income'}
                                {tx.pending && <span style={{ color: th.warn }}> · pending</span>}
                              </div>
                            </div>
                      ...
                      <div style={{ padding: '12px 0 14px', textAlign: 'center', borderTop: `0.5px solid ${th.line}` }}>
                        <span style={{ fontSize: 12, color: th.accent, fontWeight: 500 }}>See all {(txByAcct[a.id] || []).length} transactions →</span>
                      </div>
                    </div>
                  </div>
                )}
