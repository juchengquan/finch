...
function WebShell({ th, active, title, sub, actions, children }) {
  return (
    <div style={{ height: '100%', background: th.paper, color: th.ink, fontFamily: th.body, display: 'flex', overflow: 'hidden' }}>
...
// ─────────────────────────────────────────────────────────────
// 1 · Accounts overview — grouped (Cash / Credit / Investments) +
// cross-account activity merged below
// ─────────────────────────────────────────────────────────────
  ...
  return (
    <WebShell th={th} active="accts" title="Accounts" sub="4 linked · synced 9:41 AM"
      ...
      <div style={{ padding: 32 }}>
        ...
        {groupedAccounts.map((g) => {
          ...
          return (
            <div key={g.id} style={{ marginBottom: 28 }}>
              <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', marginBottom: 14, paddingBottom: 8, borderBottom: `0.5px solid ${th.line}` }}>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 12 }}>
                  <div style={{ fontFamily: th.display, fontSize: 22, fontStyle: 'italic', letterSpacing: -0.2 }}>{g.name}</div>
                  <span style={{ fontFamily: th.mono, fontSize: 10, color: th.muted, letterSpacing: 0.8 }}>{g.accounts.length} ACCOUNT{g.accounts.length > 1 ? 'S' : ''}</span>
                </div>
                <div style={{ fontFamily: th.body, fontSize: 18, fontWeight: 500, color: groupTotal < 0 ? th.neg : th.ink, fontVariantNumeric: 'tabular-nums' }}>
                  {groupTotal < 0 ? '−' : ''}<Money value={Math.abs(groupTotal)} currency={th.currency}/>
                </div>
              </div>
        ...
        {/* Activity merged */}
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 14, marginTop: 8 }}>
...
// ─────────────────────────────────────────────────────────────
// Settings · desktop
// Left-rail sub-navigation (settings categories) inside the WebShell so it
// still shares the global sidebar. Mirrors the iOS Settings screen.
// ─────────────────────────────────────────────────────────────
function WebSettings({ th }) {
  const sections = [
    { id: 'profile',    label: 'Profile',         icon: 'tag' },
    { id: 'prefs',      label: 'Preferences',     icon: 'cog',     active: true },
    { id: 'tabs',       label: 'Tab layout',      icon: 'menu' },
  ...
  const Section = ({ title, items }) => (
    <div style={{ marginBottom: 26 }}>
      <div style={{ fontFamily: th.mono, fontSize: 10, letterSpacing: 1.2, color: th.muted, textTransform: 'uppercase', marginBottom: 8 }}>{title}</div>
      <div style={{ background: th.card, border: `1px solid ${th.line}`, borderRadius: 12, overflow: 'hidden' }}>
        {items.map((it, i) => (
          <div key={it.l} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', borderTop: i ? `0.5px solid ${th.line}` : 'none' }}>
            <div style={{ flex: 1, fontSize: 14, fontWeight: 500 }}>{it.l}</div>
            {it.h && <div style={{ fontSize: 12, color: th.muted, maxWidth: 320, textAlign: 'right' }}>{it.h}</div>}
            {it.toggle != null ? (
  ...
  return (
    <WebShell th={th} active={null} title="Settings" sub="Customize Finch · synced across devices">
      <div style={{ padding: 32, display: 'grid', gridTemplateColumns: '220px 1fr', gap: 32 }}>
        {/* Left rail */}
        <div style={{ background: th.card, border: `1px solid ${th.line}`, borderRadius: 14, padding: 8, height: 'fit-content' }}>
          {sections.map((s) => (
            <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 12px', borderRadius: 8, background: s.active ? th.paperAlt : 'transparent', color: th.ink, fontSize: 13, fontWeight: s.active ? 500 : 400 }}>
              <Icon name={s.icon} size={15} style={{ color: s.active ? th.ink : th.muted }}/>
              {s.label}
              {s.active && <Icon name="chev" size={12} style={{ marginLeft: 'auto', color: th.muted }}/>}
            </div>
          ))}
        </div>
        {/* Content */}
        <div style={{ maxWidth: 720 }}>
          {/* Profile header */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 18, padding: '0 0 24px', marginBottom: 22, borderBottom: `1px solid ${th.line}` }}>
            <MerchantGlyph name="Alex Morgan" size={72} bg={th.accent} fg="#fff"/>
            <div style={{ flex: 1 }}>
              <div style={{ fontFamily: th.display, fontSize: 28, letterSpacing: -0.4 }}>Alex Morgan</div>
              <div style={{ fontSize: 13, color: th.muted, marginTop: 2 }}>alex@morgan.co · Personal plan · Member since Jan 2024</div>
            </div>
            <div style={{ height: 36, padding: '0 14px', display: 'flex', alignItems: 'center', gap: 6, background: th.paperAlt, borderRadius: 18, fontSize: 13 }}><Icon name="edit" size={13}/>Edit profile</div>
          </div>
          <Section title="Preferences" items={[
            { l: 'Default currency',  v: th.currency, h: 'Used across all amounts and reports' },
            { l: 'Categories',        v: '8 active' },
            { l: 'Tab layout',        v: '5 tabs', h: 'Customize what shows in your mobile tab bar' },
            { l: 'Date format',       v: 'May 24, 2026' },
            { l: 'Week starts on',    v: 'Monday' },
          ]}/>
          <Section title="Notifications" items={[
            { l: 'Push notifications', toggle: true },
            { l: 'Budget alerts',      toggle: true, h: 'Notify when a category nears its budget' },
            { l: 'Weekly digest',      toggle: true, h: 'Every Sunday morning, last week summary' },
            { l: 'Large transactions', toggle: false, h: 'Notify for transactions over $200' },
          ]}/>
          <Section title="Data & sync" items={[
            { l: 'Linked accounts',  v: '4' },
            { l: 'Auto-categorize',  toggle: true, h: 'AI categorization of new transactions' },
            { l: 'Sync',             v: 'iCloud' },
            { l: 'Export CSV',       v: '' },
          ]}/>
          <div style={{ textAlign: 'center', padding: '20px 0 8px', fontFamily: th.mono, fontSize: 10, color: th.muted, letterSpacing: 0.8 }}>
...
// ─────────────────────────────────────────────────────────────
// Tab layout · desktop (settings sub-page)
// ─────────────────────────────────────────────────────────────
function WebTabLayoutPage({ th }) {
  const ALL = [
    { id: 'accounts',  icon: 'wallet',   label: 'Accounts',      desc: 'Balances + activity per account', pinned: false, active: true },
    { id: 'budgets',   icon: 'target',   label: 'Budgets',       desc: 'Spending by category vs plan', active: true },
    { id: 'add',       icon: 'plus',     label: 'Add',           desc: 'Center button · always pinned', pinned: true, active: true },
    { id: 'scheduled', icon: 'calendar', label: 'Scheduled',     desc: 'Calendar of bills, subs, income', active: true },
    { id: 'insights',  icon: 'chart',    label: 'Insights',      desc: 'Analytics and patterns', active: true },
  ...
  const available = ALL.filter((t) => !t.active);
  return (
    <WebShell th={th} active={null} title="Tab layout" sub="Five tabs always show in your mobile tab bar · drag to reorder or swap">
      <div style={{ padding: 32, display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24, maxWidth: 1100 }}>
        {/* Active */}
        <div>
          <div style={{ fontFamily: th.mono, fontSize: 10, letterSpacing: 1.2, color: th.muted, textTransform: 'uppercase', marginBottom: 8 }}>In your tab bar · 5</div>
          <div style={{ background: th.card, border: `1px solid ${th.line}`, borderRadius: 14, overflow: 'hidden' }}>
            {active.map((tab, i) => (
              <div key={tab.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', borderTop: i ? `0.5px solid ${th.line}` : 'none', background: tab.pinned ? th.paperAlt : 'transparent' }}>
                ...
                <div style={{ width: 34, height: 34, borderRadius: 17, background: th.paperAlt, display: 'flex', alignItems: 'center', justifyContent: 'center', color: th.ink }}>
                  <Icon name={tab.icon} size={15}/>
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 14, fontWeight: 500, display: 'flex', alignItems: 'center', gap: 6 }}>
                    {tab.label}
                    {tab.pinned && <span style={{ fontFamily: th.mono, fontSize: 9, color: th.muted, background: th.card, padding: '1px 6px', borderRadius: 4, border: `0.5px solid ${th.line}`, letterSpacing: 0.5 }}>PINNED</span>}
                  </div>
                  <div style={{ fontSize: 12, color: th.muted, marginTop: 1 }}>{tab.desc}</div>
                </div>
                {!tab.pinned && (
          ...
          <div style={{ marginTop: 16, padding: 14, background: th.paperAlt, borderRadius: 10, fontSize: 12, color: th.ink2, lineHeight: 1.4 }}>
            <b>Preview</b> · this is what the mobile tab bar will look like:
            <div style={{ marginTop: 10, height: 56, borderRadius: 28, background: th.card, border: `1px solid ${th.line}`, display: 'flex', alignItems: 'center', justifyContent: 'space-around', padding: '0 12px' }}>
              {active.map((tab) => tab.id === 'add' ? (
        ...
        {/* Available */}
        <div>
          <div style={{ fontFamily: th.mono, fontSize: 10, letterSpacing: 1.2, color: th.muted, textTransform: 'uppercase', marginBottom: 8 }}>Available · tap to add</div>
          <div style={{ background: th.card, border: `1px solid ${th.line}`, borderRadius: 14, overflow: 'hidden' }}>
            {available.map((tab, i) => (
              <div key={tab.id} style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 16px', borderTop: i ? `0.5px solid ${th.line}` : 'none' }}>
                <div style={{ width: 34, height: 34, borderRadius: 17, background: th.paperAlt, display: 'flex', alignItems: 'center', justifyContent: 'center', color: th.ink }}>
                  <Icon name={tab.icon} size={15}/>
                </div>
                <div style={{ flex: 1 }}>
          ...
          <div style={{ marginTop: 16, fontFamily: th.mono, fontSize: 10, color: th.muted, letterSpacing: 0.6, lineHeight: 1.5 }}>
            DESKTOP TIP: the sidebar always shows all tabs (default + MORE), so customization is purely a mobile concern.
          </div>
        </div>
      </div>
    </WebShell>
...
// ─────────────────────────────────────────────────────────────
// Transaction detail · desktop (drill-in page)
// ─────────────────────────────────────────────────────────────
function WebTxDetail({ th }) {
  const tx = MOCK.transactions[1]; // Whole Foods
  const cat = catById(tx.category);
  return (
    <WebShell th={th} active="activity" title="Transaction" sub="Drill-in"
      actions={<div style={{ height: 36, padding: '0 14px', display: 'flex', alignItems: 'center', gap: 6, background: th.paperAlt, borderRadius: 18, fontSize: 13 }}><Icon name="dots" size={14}/>More</div>}>
      <div style={{ padding: 32 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: th.muted, marginBottom: 22 }}>
          <span>Activity</span><Icon name="chev" size={11}/><span>May 23</span><Icon name="chev" size={11}/><span style={{ color: th.ink }}>{tx.merchant}</span>
        </div>

        {/* Hero */}
        <div style={{ background: th.card, border: `1px solid ${th.line}`, borderRadius: 16, padding: 32, marginBottom: 22, display: 'grid', gridTemplateColumns: '110px 1fr auto', gap: 28, alignItems: 'center' }}>
          <MerchantGlyph name={tx.merchant} size={110} hue={cat.hue}/>
          <div>
            <div style={{ fontFamily: th.display, fontStyle: 'italic', fontSize: 16, color: th.muted, marginBottom: 4 }}>You spent at</div>
            <div style={{ fontFamily: th.display, fontSize: 38, letterSpacing: -0.8, lineHeight: 1 }}>{tx.merchant}</div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginTop: 12, fontSize: 13, color: th.muted }}>
              <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><CatDot hue={cat.hue}/>{cat.name}</span>
              <span>Fri, May 23 · 6:42 PM</span>
              <span>{acctById(tx.account).name}</span>
            </div>
          </div>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontFamily: th.display, fontSize: 64, letterSpacing: -2.4, lineHeight: 1 }}>
              <Money value={tx.amount} currency={th.currency} mono={false} style={{ fontFamily: th.display }}/>
            </div>
            <div style={{ fontFamily: th.mono, fontSize: 11, color: th.muted, marginTop: 8, letterSpacing: 0.5 }}>POSTED · {fmtMoney(0, th.currency).replace(/0/g, '').replace(/\./g, '')} USD</div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: 10, marginBottom: 22 }}>
          {[
            { i: 'split',  l: 'Split with someone' },
            { i: 'tag',    l: 'Recategorize' },
            { i: 'sync',   l: 'Mark as recurring' },
            { i: 'cam',    l: 'Attach receipt' },
            { i: 'edit',   l: 'Edit details' },
          ].map((a) => (
            <div key={a.l} style={{ flex: 1, height: 60, borderRadius: 12, border: `1px solid ${th.line}`, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, fontSize: 12, fontWeight: 500, color: th.ink, background: th.card }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
          <div style={{ background: th.card, border: `1px solid ${th.line}`, borderRadius: 14, padding: 20 }}>
            <div style={{ fontFamily: th.mono, fontSize: 10, color: th.muted, letterSpacing: 1.2, marginBottom: 12 }}>DETAILS</div>
            {[
              ['Category',     <span style={{ display: 'flex', alignItems: 'center', gap: 6 }}><CatDot hue={cat.hue}/>{cat.name}</span>],
              ['Account',      acctById(tx.account).name + ' · ' + acctById(tx.account).last4],
              ['Date',         'Fri, May 23 2026 · 6:42 PM'],
              ['Status',       'Posted'],
              ['Method',       'Tap to pay · physical card'],
              ['Reference',    'AMX-9F2B-44A1'],
              ['Note',         tx.note],
            ].map(([l, v], i) => (
              <div key={l} style={{ display: 'flex', justifyContent: 'space-between', padding: '11px 0', borderTop: i ? `0.5px solid ${th.line}` : 'none', fontSize: 13 }}>
                <span style={{ color: th.muted }}>{l}</span>
                <span>{v}</span>
              </div>
            ))}
          ...
          <div style={{ background: th.card, border: `1px solid ${th.line}`, borderRadius: 14, padding: 20 }}>
            <div style={{ fontFamily: th.mono, fontSize: 10, color: th.muted, letterSpacing: 1.2, marginBottom: 12 }}>YOUR HISTORY · {tx.merchant.toUpperCase()}</div>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 16 }}>
              <div>
                <div style={{ fontSize: 12, color: th.muted }}>4 visits in May</div>
                <div style={{ fontFamily: th.display, fontSize: 28, letterSpacing: -0.5, marginTop: 2 }}><Money value={184.62} currency={th.currency} mono={false} style={{ fontFamily: th.display }}/></div>
              </div>
              <div style={{ fontSize: 12, color: th.pos, fontWeight: 500 }}>↓ 12% vs Apr</div>
            </div>
            <BarChart values={[42,38,84,21,52,40,60,38]} labels={['M3','M10','M17','M23','A4','A12','A19','A26']} width={420} height={80} color={th.muted} highlight={th.accent} muted={th.paperAlt}/>
          </div>
        </div>
      </div>
    </WebShell>
...
Object.assign(window, { WebSettings, WebTabLayoutPage, WebTxDetail });
