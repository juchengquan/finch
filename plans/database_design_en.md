...
A standalone catalog of canonical merchant names — **global** since schema 2026-07-20: one shared catalog across every ledger, so there is no `ledger_id`. Linked back from `entries.counterparty_id` (was `transactions.counterparty_id` in v2; SET NULL on delete); see §6.9 + decision #18. Used by the `/merchants` admin screen.
...
CREATE INDEX idx_counterparty_name ON counterparties(name);
```
...
-- Counterparties (name resolution on every transaction write; global catalog)
CREATE INDEX idx_counterparty_name ON counterparties(name);
