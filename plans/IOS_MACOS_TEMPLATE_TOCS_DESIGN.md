...
>   `plans/IOS_MACOS_COHERENCE_PASS_DESIGN.md` — adding 8-section
...
1. **TOC presence test**: `rg "^## §0\. Map" plans/IOS_MACOS_PHASE_*_DESIGN.md | wc -l`
...
3. **TOC completeness test**: each TOC has 8 rows. Run
   `rg "^| §[0-9]\. " plans/IOS_MACOS_PHASE_*_DESIGN.md -A 100 | grep "§0\. Map" | wc -l`
...
`plans/IOS_MACOS_INDEX.md` §2 listing the 8-section template. This
