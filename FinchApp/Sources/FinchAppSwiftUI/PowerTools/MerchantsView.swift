...
struct MerchantsView: View {
    ...
    private func mergeTxCount(_ a: Counterparty, _ b: Counterparty) -> Int {
        ...
        let ids = Set(Selectors.merchantTransactions(store.txns, store.merchants, a.id, ledger, includePending: true).map(\.purchaseKey))
            .union(Selectors.merchantTransactions(store.txns, store.merchants, b.id, ledger, includePending: true).map(\.purchaseKey))
        return ids.count
    }
    private func mergeManyTxCount(_ cps: [Counterparty]) -> Int {
        let ledger = store.activeLedgerId
        var ids = Set<String>()
        for c in cps { ids.formUnion(Selectors.merchantTransactions(store.txns, store.merchants, c.id, ledger, includePending: true).map(\.purchaseKey)) }
        return ids.count
    }
}
