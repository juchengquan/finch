...
/// The row and the two write sheets for investment positions.
///
/// These lived inside `HoldingsView.swift` while the standalone Holdings screen was
/// the only thing that used them. The account detail screen now shows the same
/// positions with the same affordances, so they moved here — a screen and the pieces
/// two screens share are different things, and the standalone screen is going away.
///
/// Nothing here is iOS-only: `FinchMac` compiles this file too.
...
struct AddHoldingSheet: View {
    ...
    var body: some View {
        NavigationStack {
            Form {
                // Opened from inside an account, the caller passes that one account and
                // the picker disappears — you already said which account by being there.
                // `onAppear` selects `accounts.first` either way, so the single-account
